...
if [ ! -e ./${seq}_faa.cm8a ] ; then
    echo "Running NNcon to get contact map using ab-initio and 'homology'"
    export PATH=~jr/contrib/nncon/bin:$PATH
    LD_LIBRARY_PATH=~jr/contrib/nncon/lib \
    predict_homo_cm.sh ${seq}.faa ./
fi


# Run Scratch to predict secondary structure
if [ ! -e ${seq}.ss8 ] ; then
...
if [ "$FOLD" == "UNICON3D" ] ; then
    ...
    #mv ${seq}* nncon+homo
    #cp nncon+homo/${seq}.faa .
fi
exit
# THESE REQUIRE AN RR FILE!!
