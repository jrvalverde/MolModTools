...
if [ ! -s $out/starting_models/$out/af_model_1.pdb ] ; then
    ...
    for (( i=0; i < $maxmodels; i++ )); do
        ...
        elif [ ! -s $out/starting_models/$out/af_model_1.pdb ] ; then
...
# ------
# Ditto.
# We should aim for consistency in model naming here, and call
# the models $out/starting_models/dncon2_model$i.pdb !!!
#
if [ ! -s $out/starting_models/dncon2_model_1.pdb ] ; then
# was if [ ! -s $out/starting_models/1.dncon2.*.pdb ] ; then
...
# Select a suitable contact matrix
# we should prefer alphafold, c-quark, tr-rosetta and dncon2
