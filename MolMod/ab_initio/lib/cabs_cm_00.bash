...
function cabs_cm_00() {
    ...
    if [ -s $cm ] ; then
        cp $cm $workdir/contactmap.rr5
    elif [ -s contactmap.rr5 ] ; then
        cp contactmap.rr5 $workdir/contactmap.rr5
    else
        warncont "called with no contact matrix (in RR5 format)"
    fi
    
    # Check if the $pdb varaible exists and isn't empty and whether 
    # it is a file or a directory
    if [ "$pdb" != "" -a -d $pdb ] ; then 
        # if it is a directory, copy all pdb file/cabs to workdir (cabs)
        cp $pdb/*pdb $workdir
    elif [ -s $pdb ] ; then
        # if it is a file, assume it is a PDB file to refine and copy it
        cp $pdb $workdir
    else
