...
sequence=$1	#protein sequence ex: ../seq/"1CTF.fasta"
cm=$2 		#contact map ex: ../dncon2/1CTF/out/1CTF.rr.raw
	
filename=${sequence##*/} #proteinname.fasta
...
cd ${name}
        if [ ! -e score.apollo ] ; then
                mkdir -p score.apollo/models
                for k in */output_pdbs/model*.pdb ; do
                	ls $k \
                        | while read mod ; do 
    				dest=score.apollo/models/`echo $mod | tr '/' '-'`
    				#echo cp $mod $dest
    				if [ ! -e $dest ] ; then
    			    		cp $mod $dest
                        	fi
