...
function rmsd_vs_ref() {
    ...
    if [ "$re" != "pdb" ] ; then
        if [ "$re" == "gro" ] ; then
            if [ -e "$rn.tpr" ] ; then
                echo 0 | gmx trjconv -f "$rf" -s "$rn.tpr" -o "$rn.pdb"
            else
...
#
# analyze a trajectory (possibly split in many pieces)
#
...
if [ ! -e ${md}_noPBC.xtc ] ; then
    echo System | gmx trjconv -f ${md}.xtc -s ${md}.tpr -o ${md}_noPBC.xtc \
    	-pbc mol -ur compact
fi
...
#-----------------------
BASE=`dirname $0`
$BASE/analyze_md_run.sh ${md} 2>&1 | tee ${md}_analysis.log
