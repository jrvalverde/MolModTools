...
#
# NAME
#	gmx_extend_trj_until_ps - extend a trajectory to time in ps in new files
#
# SYNOPSIS
#	gmx_extend_trj_until_ps [name] [final_ps]
#
# DESCRIPTION
#	Given a terminated trajectory, with its corresponding input run 
# .tpr and checkpoint .cpt files, create a new extension trajectory until
# the specified time in picoseconds is reached.
#
...
#	final_ps	- the time in ps when the new trajectory will finish
#
# LICENSE:
#
...
GMX_EXTEND_TRJ_UNTIL_PS_H="GMX_EXTEND_TRJ_UNTIL_PS_H"
...
function gmx_extend_trj_until_ps() {
    if ! funusage $# [md_name] [final_ps] ; then return $ERROR ; fi 
    notecont "$*"

    local name=${1:-md}
    local final_ps=${2:1000}
    local ERR=1
    ...
    # 
    previous="$name"
    next="${previous}_to_${final_ps}ps}"
    ...
    gmx convert-tpr -s "$previous.tpr" -until $final_ps -o "$next.tpr" -v
...
if [[ $0 == $BASH_SOURCE ]] ; then
    ...
    gmx_extend_trj_until_ps $*
else
    export GMX_EXTEND_TRJ_UNTIL_PS_H
fi
