...
#
ANALYZE_MD_RUN='ANALYZE_MD_RUN'		# to know we have been sourced

LIB=`dirname ${BASH_SOURCE[0]}`	# located in our same directory
source $LIB/include.bash
include setup_cmds.bash
include util_funcs.bash
include make_ext_index.bash
...
function analyze_md_run {
    if ! funusage $# MD-run-base-name [ligands-file] ; then return $ERROR ; fi
    notecont $*
    ...
    if [ ! -e $md.trr ] ; then
    	warncont "No trajectory found for MD run \"$md\"."
        return $ERROR
    fi
    if [ -e ${md}.analysed ] ; then
        notecont "MD run \"${md}\" is already analyzed"
        notecont "remove file $md.analysed to re-analyze"
        return $OK
    fi
    
    # ensure we have definitions of solute, solvent and chains
    if [ ! -e $md.ndx ] ; then
        notecont "Building index"
        make_ext_index $md
    fi
    
    if [ ! -e ${md}.xtc ] ; then
        notecont "Creating XTC file"
    ...
    if [ ! -s lasttime ] ; then
        notecont "Finding last time in trajectory"
        ...
        if [ "$lastt" == "" ] ; then 
            warncont "NO LAST FRAME! (lastt='$lastt')" 
            return $ERROR 
        fi
        notecont "LAST TIME = $lastt" 
        echo "$lastt" > lasttime
    else
        lastt=`cat lasttime`
    fi
    ...
    if [ ! -e $md.gro ] ; then
        notecont " Getting last configuration"
    ...
    if [ ! -e ${md}_complex_fit.xtc ] ; then
        notecont "Fitting system to itself"
...
END
    ...
    #
    notecont "Computing radius of gyration"
    ...
    #
    notecont "Computing secondary structure changes"
    #
...
END
    ...
    if [ -s $ligands ] ; then
        echo ""
        notecont ">>> Analyzing ligands and ligand interactions"
        echo ""
...
END
    ...
    notecont "Doing any remaining plot"
