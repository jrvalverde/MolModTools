...
#	MAYBE WE SHOULD ITERATE 10 TIMES INSTEAD AND USE A DECIDER VARIABLE?
#
...
#source ~/contrib/gromacs-2021/bin/GMXRC.bash
...
if [ ! -e step10_1000x.trr -a "$HUMONGOUS" == "TRUE" ] ; then
   # multiply by 100 the number of steps and output saving intervals
    cat step7_production.mdp \
        | sed -e '/^nsteps / s/$/000/g' \
        > step10_1000x.mdp
    domd step10_1000x step9_100x
# if you add this it will divide output frames by 10
#        | sed -e '/^nst.*out/ s/$/0/g' \
# if you add this it will divide output frames by 100
#        | sed -e '/^nst.*out/ s/$/00/g' \
# if you add this it will divide output frames by 1000
#        | sed -e '/^nst.*out/ s/$/000/g' \
# it is better not to and, if required, change the stride on loading the
# trajectory.
    ...
    echo "INFO: step10_1000x.trr already exists. Skipping"
fi
