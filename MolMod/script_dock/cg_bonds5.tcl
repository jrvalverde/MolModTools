...
proc cg_bonds_usage { } {
  ...
  puts "   -gmx       /usr/bin/gmx         path to the \"gmx\" executable"
  ...
  puts "   cg_bonds -gmx gmx -tpr mini.tpr"
  puts "   cg_bonds -gmx /usr/local/gromacs-2016.3/bin/gmx -topoltype \"martini\" -top system.top -cutoff 6.2"
...
proc parse_gmxdump_output { molid gmxdump tpr topology_type BD ELAST CNSTR } {
  ...
  puts "going to dump tpr "
   set tpr [open "| $gmxdump dump -s $tpr 2> /dev/null | grep -F -e \#atoms -e \#beads -e moltype -e \#molecules -e \(BONDS\) -e \(CONSTR\) -e \(HARMONIC\)" "r"]

  puts "variable tpr set "
  puts $tpr

  while { [gets $tpr line] > 0 } {

    puts "got another line "
