...
if [ -e chim_${f}_amber/${f}_amber.pdb ] ; then echo "$f already done" ; exit ; fi
...
if [ ! -s chim_${f}_amber/${f}_amber.pdb ] ; then
    ...
    DISPLAY='' chimera --nogui >& chim_${f}_amber/${f}_chimera.log <<END
        open $pdb
        # Prepare:
        ...
        minimize nogui true prep true nsteps 5000 cgsteps 500 freeze none interval 100
