...
function analyze_md_run {
    ...
    # fix trajectory using first frame as reference
    if [ ! -e ${md}_fit.xtc ] ; then
        echo "System" | $trjconv -f $md.xtc -s $md.tpr -n $md.ndx \
		-o ${md}_fit.xtc -fit rot+trans
...
############### XXX JR XXX ################
# REWRITE FILE TO CARRY OUT EACH ANALYSIS ON A SEPARATE SUBDIRECTORY
if [ "TRUE" == "FALSE" ] ; then
	...
	gmx covar -s ${md}_complex.tpr -f ${md}_complex.xtc \
	    -n ${md}_complex.ndx \
            -xpm ${md}_covar.xpm -xpma ${md}_covar_aa.xpm \
	    -o ${md}_eigenval.xvg -av ${md}_covar_average.pdb \
	    -v ${md}_eigenvec.trr \
	    <<END
Backbone
Backbone
END
    ...
    if [ -s ${md}_eigenvec.trr ] ; then
        # analyze eigenvectors 1 to 4 (PCA dimensions 1:4)
        gmx anaeig -s ${md}_complex.tpr -f ${md}_complex.xtc \
	    -v ${md}_eigenvec.trr -eig ${md}_eigenval.xvg \
	    -proj ${md}_proj_ev1.xvg -extr ${md}_ev1.pdb \
	    -rmsf ${md}_rmsf_ev1.xvg \
	    -first 1 -last 5 \
	    <<END
Backbone
Backbone
END
    ...
    gmx chi -s -s ${md}_complex.tpr -f ${md}_complex.xtc -all -maxchi 2
fi
##########################################
