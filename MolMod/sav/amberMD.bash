...
integrator	= cg ;steep	; Algorithm options. cg = conjugate gradients
...
; 7.3.5 Energy Minimization
emtol		= 10.0		; Stop minimization when the energy changes by less than emtol kJ/mol/nm.
emstep          = 0.01          ; [nm] initial step-size
nstcgsteep      = 10            ; do a steep every 10 steps of cg
...
if [ -e em.gro -a ! -e em.pdb ] ; then $trjconv -f em.gro -s em.tpr -o em.pdb -pbc nojump ; fi
if [ -e nvt.gro -a ! -e nvt.pdb ] ; then $trjconv -f nvt.gro -s nvt.tpr -o nvt.pdb -pbc nojump ; fi
if [ -e npt.gro -a ! -e npt.pdb ] ; then $trjconv -f npt.gro -s npt.tpr -o npt.pdb -pbc nojump ; fi
if [ -e md.gro -a ! -e md.pdb ] ; then $trjconv -f md.gro -s md.tpr -o md.pdb -pbc nojump ; fi

# if that failed, tray with editconf
# editconf loses chain information because .gro lacks it, it will also keep PBC
if [ -e em.gro -a ! -e em.pdb ] ; then $editconf -f em.gro -o em_PBC.pdb ; fi
if [ -e nvt.gro -a ! -e nvt.pdb ] ; then $editconf -f nvt.gro -o nvt_PBC.pdb ; fi
if [ -e npt.gro -a ! -e npt.pdb ] ; then $editconf -f npt.gro -o npt_PBC.pdb ; fi
if [ -e md.gro -a ! -e md.pdb ] ; then $editconf -f md.gro -o md_PBC.pdb ; fi
