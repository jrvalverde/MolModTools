...
. $FUNCS_BASE/gromacs_funcs.bash
. $FUNCS_BASE/fs_funcs.bash
. $FUNCS_BASE/util_funcs.bash
INCLUDE='no'
...
topology='default'	# default topology to use (none = use hardcoded heuristic)
...
if ! extract_ligands Protein.pdb $ligands $chargeModel $topology ; then
...
if [ ! -e ProteinOK.pdb ] ; then
    ...
    eval $pdb2gmx -ff oplsaa -f Protein.pdb -o ProteinOK.pdb -p Protein.top \
...
if [ ! -e Complex.pdb -o ! -e Complex.top ] ; then
    ...
    echo ""
    echo "Using Complex.pdb and Complex.top from previoys run."
    echo ""
fi
echo ""
