...
#include kk.sh
# it is better not to use aliases and we do not need INCLUDE any more
...
source $FUNCS_BASE/get_pdb_dimensions.sh
...
dist='1.2'		# distance from system to box (in nm)
xbox=0			# box size in the X dimension (nm)
ybox=0			# box size in the Y dimension (nm)
zbox=0			# box size in the Z dimension (nm)
...
#export openmopac=~/contrib/mopac/MOPAC2009.exe	# should use this but no longer works
export openmopac="${HOME}/contrib/mopac/MOPAC2016.exe"
export MOPAC_LICENSE=${HOME}/contrib/mopac
        ...
        Parameters:
        	...
        	-i	input file in PDB format
                ...
                -p      pH to consider when adding H (only used if 
                	method is babel)
		-d	distance from system to PBC box [1.2]
		-b	box size (disables -d) as X,Y,Z
...
TEMP=`getopt -o c:t:i:h:p:b:¡l:sSnOEMLv \
     --long charge:,topology:,input:,addH:,ph:,box:,ligands:,solvate,saline,neutral,optimize,equilibrate,shortmd,longmd,verbose \
     -n "$0" -- "$@"`
...
while true ; do
	case "$1" in
		...
		-h|--addH) 
		    ...
		    case "$2" in
		        chimera|babel|gromacs|reduce|haad|none)
                            addHmethod=$2 ;;
                        *)
			    echo "ERROR: requested method <$2> is not supported"
			    usage ; exit ;;
                    esac
                    shift 2 ;;
                -p|--pH)
                    # check $2 is a positive real number
		    # 11 11. 11.10 11.10e10 11e10.10 -11.10e10
                    if [[ $2 =~ ^[+]?[0-9]+([.eE][0-9]*)*$ ]] ; then
                        pH=$2
                    else
                        echo ""
		...
		-d|--distance)
                    ...
                    shift 2 ;;
		-b|--box)
		    read x y z <<<$(echo "$2" | tr ',' ' ')
		    # check $x is a  real number
		    if [[ $x =~ ^[+-]?[0-9]+([.eE][0-9]*)*$ ]] ; then
                        xbox=$x
                    else
                        echo ""
                        echo "ERROR: $x is not a positive real number."
                        usage ; exit 1
                    fi
                    # check $y is a positive real number
                    if [[ $y =~ ^[+-]?[0-9]+([.eE][0-9]*)*$ ]] ; then
                        ybox=$y
                    else
                        echo ""
                        echo "ERROR: $y is not a positive real number."
                        usage ; exit 1
                    fi
                    # check $z is a positive real number
                    if [[ $z =~ ^[+-]?[0-9]+([.eE][0-9]*)*$ ]] ; then
                        zbox=$z
                    else
                        echo ""
                        echo "ERROR: $z is not a positive real number."
                        usage ; exit 1
                    fi
		    # disable dist (we are giving an explicit box)
		    dist="0"
                    shift 2 ;;
		-s|--solvate)
                    # add water
...
if [ "$dist" != "0" ] ; then
    # try to estimate box size
    read x y z <<<$( get_pdb_dimensions $input size )
    
    xbox=`echo "scale=3; ($x + 24)/10" | bc -l`
    ybox=`echo "scale=3; ($y + 24)/10" | bc -l`
    zbox=`echo "scale=3; ($z + 24)/10" | bc -l`
    # these values will not be used if $dist 1= '0'
    # they are just orientative for output
fi
...
#
#	The trick is that LOG is defined in gromacs_funcs.bash as ''
...
#	the original definition of LOG in gromacs_funcs.bash took place
#	before this one (on initialization) and is superseded by this.
#	If we do not re-define LOG, then gromacs_funcs will work as if it
#	did not exist
#
if [ ! -e ./$name/log.$myname.out ] ; then touch ./$name/log.$myname.out ; fi
if [ ! -e ./$name/log.$myname.err ] ; then touch ./$name/log.$myname.err ; fi
stdout=`abspath ./$name/log.$myname.out` 
stderr=`abspath ./$name/log.$myname.err` 
version_file $stdout
version_file $stderr
...
# we could get a PDB ID if needed
...
if ! extract_ligands Protein.pdb $ligands $topology $ff ; then
    echo "ERROR: Could not extract ligands"
    exit $ERROR
fi
...
if [ ! -e ProteinOK.pdb ] ; then
    ...
    if [ "$ff" == "opls-aa" ] ; then forcefield="oplsaa" ; fi
...
if [ ! -e Complex_PBC.top ] ; then
	...
	# IMPORTANT!
	#
	# NOTE the magic number (1.2 nm) This should match the values in MDP files
	# -d is the distance between the solute and the box
	# Note that Gromacs may take shortcuts to exploit PBC
	eval $editconf -bt triclinic -d 1.2 \
	               -f Complex.pdb -o Complex_PBC.pdb $LOG
    else # we are using a specified box size
	# IMPORTANT
	# ONLY USE IF YOU KNOW WHAT YOU ARE DOING!!!
	eval $editconf -bt triclinic -box $xbox $ybox $zbox \
		       -f Complex.pdb -o Complex_PBC.pdb $LOG
    fi
...
for i in *.gro ; do
    ...
    # .gro does not have chain ID, which is why we need the .tpr as reference
    if [ ! -s `basename $i gro`pdb ] ; then 
        $trjconv -f $i -s `basename $i gro`tpr -o `basename $i gro`pdb
    fi
    # if that failed then try this (will lose chain info)
    if [ ! -s `basename $i gro`pdb ] ; then 
...
if [ "$grace" = "" ] ; then
    ...
    for i in *.xvg ; do
       if [ ! -e `basename $i xvg`svg ] ; then
            $grace $i \
            	-hardcopy -hdevice SVG \
            	printfile `basename $i xvg`svg \
            	-pexec 'runavg(S0,100)'
        fi
    done
fi
