...
function docker {
    ...
    local PATH=$jrbin:$dock6bin:$chimerabin:$pdbtools:$PATH
    ...
    if [ -s $rnam.mol2 ] ; then
        cp $receptor 01_dockprep/rec.mol2
    else
       # must be PDB
       babel -ipdb $rnam.pdb -omol 01_dockprep/rec.mol2
    fi
    if [ -s $rnam.pdb ] ; then
        cp $receptor 01_dockprep/rec.pdb
    else
        # must be a mol2
    ...
    if [ "$known" != "" ] ; then
        # KNOWN exists and is a mol2 file
...
END
    ...
    if [ ! -s rec.pdb -o ! -s rec.mol2 ] ; then
        echo "
...
#           mv OUTSPH `basename $i .dms`.OUTSPH
    ...
    if [ ! -s 05_sitsph/knl.mol2 ] ; then
        # XXX JR XXX
        #	NOTE: if a molecule has a known ligand but we do not
        #   want to fit in its pocket, we should NOT use these.
        #
        ...
        open ${base}.pdb
        select ligand
        write selected format mol2 0 05_sitsph/${base}-L.mol2
        select nucleic acid
        write selected format mol2 0 05_sitsph/${base}-N.mol2
END
    ...
    # the largest sphere cluster (the first cluster).
